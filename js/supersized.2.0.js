	...
	$.fn.resizenow = function() {
		...
		var options = $.extend($.fn.supersized.defaults, $.fn.supersized.options);
	  	return this.each(function() {
			...
			//var ratio = $(this).find("img").height()/$(this).find("img").width();
			...
			if ((browserheight/browserwidth) > ratio){	    
			    ...
			    $(this).width(Math.round(browserheight / ratio));
			    $(this).children().height(browserheight);
			    $(this).children().width(Math.round(browserheight / ratio));
			} else {
				$(this).width(browserwidth);
			    $(this).height(Math.round(browserwidth * ratio));
			    $(this).children().width(browserwidth);
			    $(this).children().height(Math.round(browserwidth * ratio));
			}
			...
			{
				$(this).children().css('top', (browserheight - $(this).height())/2);
				$(this).children().css({'left':0,'bottom':0});
				$(this).css({'left':0,'bottom':0});
				//$(this).children().hide()
				
			}
			//$("#debug").html($(this).height()+"x"+$(this).width())
			//alert("resize");
			return false;
		});
	};
